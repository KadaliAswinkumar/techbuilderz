...
export function Hero3D() {
  ...
  useEffect(() => {
    ...
    // Morphing icosahedron with wireframe
    const geo = new THREE.IcosahedronGeometry(2, 4);
    const mat = new THREE.MeshStandardMaterial({
      color: 0x6b5cff, wireframe: true, emissive: 0x4b3cff, emissiveIntensity: 0.6,
    });
    ...
    const pMat = new THREE.PointsMaterial({ color: 0x9bd8ff, size: 0.03, transparent: true, opacity: 0.8 });
    const points = new THREE.Points(pGeo, pMat);
    scene.add(points);

    scene.add(new THREE.AmbientLight(0xffffff, 0.4));
    const l1 = new THREE.PointLight(0x6b5cff, 2, 30); l1.position.set(5, 5, 5); scene.add(l1);
    const l2 = new THREE.PointLight(0x00e5ff, 2, 30); l2.position.set(-5, -3, 4); scene.add(l2);
    ...
    const animate = () => {
      ...
      mesh.rotation.y += (mx * 0.3 - mesh.rotation.y * 0.0) * 0.02;
